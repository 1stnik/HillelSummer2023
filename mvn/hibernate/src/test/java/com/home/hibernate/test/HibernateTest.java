...
public class HibernateTest
{
  ...
  @Test
   ...
   public void testUpdateOnePerson()
   {
      ...
      Person p = (Person) session.get( Person.class, 1L );
      System.out.println( "Retrieved person from DB is " + p );
      p.setFirstName( "Changing the name TEST " );
      session.update( p );
      session.getTransaction().commit();
...
//
   ...
   public void testCriteria()
   {
      ...
      criteria.add( Restrictions.or( Restrictions.eq( "name", "apple" ), Restrictions.between( "id", 3, 5 ) ) );
   ...
   public void testProjections()
   {
      ...
      Criteria criteria = session.createCriteria( Phone.class );
      criteria.setProjection( Projections.property( "name" ) ).addOrder( Order.desc( "name" ) );
